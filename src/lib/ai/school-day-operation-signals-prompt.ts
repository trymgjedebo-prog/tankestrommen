...
ADJUST_START — ATTENDANCE VS. OTHER SAME-DAY MESSAGES: an explicit time for when the pupil should MEET, or when the pupil's overall school day BEGINS, still yields "adjust_start" even when the same day paragraph ALSO contains other separate messages — a book handover, a reminder, a practical notice, or information about what some pupils did earlier. Judge whether the time semantically applies to the pupil's overall attendance / school-day start; do NOT withhold the signal merely because the paragraph has several sentences. Still do NOT emit "adjust_start" when the time instead belongs to a single item (a book handover, one exam, one lesson, an appointment, a mid-day activity), when it applies only to a subgroup or to a class/pulje the child is not clearly matched to, when it applies to some pupils but not to the pupil's overall day, or when there is no clear day-level scope. The test is always semantic: does this time mark when the pupil's overall school day begins?
...
- SHOULD emit adjust_start (attendance time stands despite a separate message) — "Elevens oppmøte kl. 10.30. Bokinnlevering for alle som har hatt eksamen." The first sentence fixes the pupil's overall attendance; the book handover is a separate message and does NOT override the start-time signal → {"operation":"adjust_start","effectiveStart":"10:30", ...}.
...
- SHOULD NOT emit adjust_start (time belongs to the book handover) — "Bokinnlevering for 2STC kl. 10.30–11.00." The time is the book handover's, not the start of the pupil's school day.
- SHOULD NOT emit adjust_start (conditional subgroup, ordinary teaching continues) — "Muntlig eksamen for noen elever kl. 10.30. Vanlig undervisning for resten." The time applies to a conditional subgroup and ordinary teaching still applies.
- SHOULD NOT emit adjust_start (a single lesson, not the pupil's overall start) — "Naturfag kl. 10.30." One lesson, not when the pupil's overall school day begins.
